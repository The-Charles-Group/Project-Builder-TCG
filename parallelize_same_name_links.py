...
def parse_minutes(d):
    if not d: return 0
    m = re.match(r"PT(\d+)M$", d)           # PT####M
    if m: return int(m.group(1))
    m = re.match(r"P(?:(\d+)D)?T(?:(\d+)H)?(?:(\d+)M)?(?:(\d+)S)?$", d)  # full ISO
    if m:
...
def makespan_days(xml_path):
    ...
    for t in tasks_el.findall(f"{{{NS}}}Task"):
        ...
        uid = int(uid_el.text)
        if uid == 0: continue  # ignore project summary row
    ...
    while q:
        ...
        for v in uids:
            if u in preds.get(v, []):
                ...
                if in_deg[v] == 0:
                    q.append(v)
    return (max(EF.values())/480.0) if EF else 0.0  # 480 min/day
...
def parallelize_same_name_links(src_path, out_path):
    ...
    # Remove only those PredecessorLinks where pred & succ share the same Name
    removed = 0
    for t in tasks_el.findall(f"{{{NS}}}Task"):
        uid_el = t.find(f"{{{NS}}}UID")
        if uid_el is None: 
