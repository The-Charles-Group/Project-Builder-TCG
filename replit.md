...
- **GPT 5.1 Pro RFP Summary**: Structured `summary_bullets` field in RfpSummary model containing deliverable labels and descriptions. Frontend `renderRfpSummary()` consumes structured bullets with fallback to colon-delimited summary_text parsing for backward compatibility with cached responses.
