...
- **Effort-Based Waterfall Scheduling**: Feature-flagged system (PHASE_WINDOW_ENABLED) implementing the Final Scheduling Ruleset. Key behaviors: (1) Leaf task duration = ceil(hours/8) business days; (2) Waterfall sequencing per component: general → internal_review → client_review → qa with FS dependencies; (3) Bottom-up rollup: parents derive dates from children, not vice versa; (4) Constraints: Deliverables get SNET, leaves use ASAP with FS dependencies. TICKET B (child stretching) disabled. XML export uses scheduler's `phase_window` metadata from compressed_timeline for accurate FS link generation.
