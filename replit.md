...
- **Pre-Export Sync Pipeline (Dec 2025)**: Complete collect→sync→export pipeline ensuring DOM edits reach exports. Implementation: (1) `collectScenarioFromUi()` REWRITTEN to deep-clone scenario and read ALL hours/rates from DOM inputs (`#hours-${code}`, `#rate-${code}` for deliverables; component rows via `[data-component]` selector), then RETURNS the updated scenario; (2) `ensureScenarioSyncedBeforeExport()` calls collectScenarioFromUi, uses returned value to update local `scen` and global `window.SCENARIO_A`, then syncs to backend; (3) All export functions (`onExportScenario`, `onExportXMLScenario`, `exportPricingDetails`, `exportScenario`, `onExport`) use `let` for scenario variables, reassign to `window.SCENARIO_A` after sync for Scenario A exports, and send the UPDATED scenario in their payloads. Critical pattern: DOM edits → collectScenarioFromUi() RETURNS updated scenario → update globals → sync to backend → export with synced data.
- **Graceful /api/scenarios Response (Dec 2025)**: Backend endpoint now returns `{ok: false, needs_step2: true}` for new sessions without working scenarios (instead of throwing 422). Frontend `initPricingStep()` handles this gracefully, showing "need to seed from Step 2" message without errors. This enables clean Step 3 initialization for both fresh and existing sessions.
- **QA Debug Endpoint (Dec 2025)**: `/api/debug/deliverable_hours/{session_id}/{deliverable_code}` returns SCENARIO_STORE data for a specific deliverable, enabling QA to verify hours match between UI and exports without mutating state. Returns: `{deliverable_code, rows, scenario_hours_sum, rows_detail[{WBS_ID, Component, Planned_Hours, Rate}]}`.
- **Component-Level Hours Tracking System (Dec 2025)**: Granular component-level pricing overrides enabling users to edit individual component hours while automatically cascading totals to parent deliverables. Key components:
  - `findScenarioItemByCode(dcode)`: Finds deliverable item in `window.SCENARIO_A.items` by code
  - `applyPricingEditToScenario(dcode, component, field, value)`: Updates `window.SCENARIO_A.items[].component_hours` map with component-level edits, recalculates parent total as sum
  - `setupPricingDetailsEventDelegation()`: Delegated change handler for Pricing Details grid edits
  - `collectScenarioFromUi()`: Now uses `window.SCENARIO_A` as authoritative source, preserves `component_hours` through clone, derives hours from `component_hours` sum when present
