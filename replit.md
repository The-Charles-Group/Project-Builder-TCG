...
- **GPT 5.1 Pro RFP Summary**: Frontend `renderRfpSummary()` parses `summary_text` (newline-separated bullets with `•` prefix) into `<ul class="rfp-summary-list">` with `<strong>Label</strong> — description` format. Both Fast and Deep modes share identical bullet rendering; only the model-generated text differs.
