...
- **Pre-Export Sync Pipeline (Dec 2025)**: All exports now call `ensureScenarioSyncedBeforeExport(scenario, letter)` before downloading. This helper: (1) Flushes pending ScenarioStore patches via `flushPatchesNow()`; (2) Collects UI metadata via `collectScenarioFromUi()`; (3) Syncs the scenario to backend via `syncScenarioToBackend()`. All export functions pass their specific scenario object and letter, ensuring the correct data is synced. Key files: `static/index.html` (onExportScenario, onExportXMLScenario), `static/app.js` (exportPricingDetails, exportScenario, onExport, exportXMLScenario). Note: Primary UI workflow uses Scenario A only.
