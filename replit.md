...
- **Workfront Duration Alignment (Dec 2025)**: Step 4 UI and MSP/XML exports now use business-day durations matching Workfront's "Duration" column:
  ...
  7. **Leaf Task Duration Business-Day Fix (Dec 2025)**: Fixed critical bug where Workfront compressed Brand Identity from 64 days to 25 days on import. Root cause: Leaf task Duration was calculated from `ceil(hours/8)` which didn't match the actual Start/Finish date span. Fix: Leaf task Duration now uses `calculate_business_days_between(start, finish) * 480` to derive Duration from business days (Mon-Fri) between Start and Finish dates. Safeguards: Force 1 day minimum if duration=0 with work, PT0M for true milestones. This ensures Workfront respects the timeline without recalculating dates.
