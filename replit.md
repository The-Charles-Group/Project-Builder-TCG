...
- **Unified Pricing Data Flow (Dec 2025)**: Single codepath for all pricing operations ensuring backend is source of truth. Implementation: (1) `syncScenarioToBackend()` POSTs to `/api/scenario/sync` to update SCENARIO_STORE; (2) `rebuildPricingFromBackend()` calls sync first (if scenario provided), then `/api/pricing/rebuild_breakdown` for authoritative totals, updating ALL summary elements (One-Time, Retainer, Grand Total) with counts, hours, and prices from backend; (3) Build Scenario button uses dual-path logic: first click calls `buildScenariosAB()` for initial creation, subsequent clicks apply pricingData edits and call `rebuildPricingFromBackend()` silently; (4) Legacy static pricing table removed - only dynamic grid remains; (5) Export triggers call `syncScenarioToBackend()` before invoking export endpoints; (6) Race condition prevention via `window._isBuildingSyncing` flag; (7) Retainer pricing uses actual `retainer_months` per deliverable for accurate monthly/total calculations (not hardcoded 12); (8) Backend returns `total_price` for retainers enabling correct grand total formula: One-Time + Retainer Total = Grand Total.
