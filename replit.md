...
- **GPT 5.1 Pro Cadence/Retainer System (Dec 2025)**: Production-ready cadence sync architecture following "let Python do the math" principle. Backend cadence helpers at lines 1056-1239: `cadence_changed()`, `cadence_price_changed()`, `monthly_price_changed()` detect field changes, `apply_cadence_to_item()` applies priority rules (monthly_price > cadence_price > cadence structure). `/api/scenario/sync` endpoint (lines 14526-14662) uses `merge_scenario_from_sync()` to load existing scenario, build old_item mapping, iterate items calling `apply_cadence_to_item`, and store via `update_working_scenario()`. Frontend sync wiring: `updateCadenceType()`, `updatePeriods()`, `updateMonthlyPrice()`, `updateCustomHours()`, `updateCustomRate()` all update `SCENARIOS.A.items` AND call `ScenarioStore.patchScenarioItem()` with appropriate fields (billing_cadence, cadence_units, retainer_months, monthly_price, cadence_price). `updatePeriods()` includes authoritative price field (monthly_price or cadence_price fallback) to maintain backend priority rules. Step 3 UI shows cadence dropdown, units input, total months readonly, monthly price input, and retainer badge. Export via `expand_retainer_into_months()` (lines 1346-1402) uses synchronized cadence values. **UI Dual-Table Architecture (Dec 2025)**: Step 3 uses two complementary rendering systems - `renderScenario()` for the main Project Scenario table (renders to `scenarioA` container, receives data as parameter) and `updatePricingTable()` for the Deliverable Pricing Details table (renders to `pricing-container`, uses global SCENARIOS). All call sites first call `renderScenario()` for immediate display, then `updatePricingTable()` for cadence editing features. Both tables sync via common data source and cadence edits flow through `ScenarioStore.patchScenarioItem()` to backend.
