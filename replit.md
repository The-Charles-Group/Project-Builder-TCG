...
- **Effort-Based Waterfall Scheduling**: Feature-flagged system (PHASE_WINDOW_ENABLED) implementing the Final Scheduling Ruleset. Key behaviors: (1) Leaf task duration = ceil(hours/8) business days; (2) Waterfall sequencing per component: general → internal_review → client_review → qa with FS dependencies; (3) Bottom-up rollup: parents derive dates from children, not vice versa; (4) Constraints: Deliverables get SNET, leaves use ASAP with FS dependencies. XML export uses scheduler's `phase_window` metadata from compressed_timeline for accurate FS link generation.
- **Unified Schedule Engine (Dec 2025)**: Single source of truth for UI Gantt + XML export. Key features: (1) ENABLE_DESCENDANT_STRETCHING flag (default ON) controls whether leaf tasks stretch to fill deliverable windows; (2) Dual metrics on every task: effort_hours (planned work) and duration_days (business-day span); (3) Task-level Gantt API endpoint (/api/gantt/task_level) returns L2→L3→L4 hierarchy; (4) UI popup shows Effort vs Duration side-by-side; (5) Milestones handled properly with 0 duration. Addresses "why is it 12 days for 3 hours" confusion by showing both metrics.
- **5.2 Pro Prescription (Dec 2025)**: Five-step scheduling compliance ensuring Duration_Days drives timeline, not PlannedHours. Key fixes: (A) build_wbs_with_pricing uses offset_by_tg/duration_by_tg for ALL nested tasks; (B) convert_excel_to_mspdi computes Finish via add_business_days, uses calculate_business_days_between for all duration calculations; (C) compressed timeline L2 dates trigger child rescheduling relative to deliverable windows; (D) /api/gantt/task_level returns ALL WBS rows with uid, parent_uid, start, finish, planned_hours, duration_business_days; (E) XML export uses business-day durations (DurationMinutes = business_days * 480) for preserved Gantt and compressed timeline paths.
- **GPT 5.2 Pro A-E Workfront Fixes (Dec 2025)**: Comprehensive Workfront import compatibility fixes based on XML analysis. (A) Export full 4-level hierarchy without OutlineLevel filtering; (B) STRIP_ASSIGNMENTS_FOR_WF=False preserves Resources/Assignments for assignment breakdowns; (C) Dangling PredecessorUID cleanup removes invalid predecessor references before validation to prevent "can't read file" errors; (D) Duration = business_minutes_between(Start, Finish) for ALL tasks including assignments (Work contains effort, Duration contains schedule span); (E) /api/gantt/task_level uses identical business-day scheduling as XML export.
- **GPT 5.2 Pro Additional Fixes (Dec 2025)**: (1) Business-day shifting in compress_deliverable_timeline uses add_business_days() instead of timedelta(days=) to prevent tasks landing on weekends; (2) MSPDI header fields SaveVersion, Title, Author added before <Name> element as required by Workfront; (3) Timeline fields (timeline_start/timeline_end/timeline_working_duration_days) now only stamped onto deliverable-level tasks (level == 2), preventing UI/Workfront drift where child tasks inherit wrong dates.
