...
async def upload_file_and_get_suggestions(file_path: str) -> Dict:
    """Upload a document and get AI suggestions using /api/suggest_by_file"""
    
    if not os.path.exists(file_path):
        raise FileNotFoundError(f"Test file not found: {file_path}")
    
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        # Upload the file to get suggestions
        with open(file_path, 'rb') as f:
            ...
            # Call the suggest_by_file endpoint
...
async def analyze_text_with_ai(text: str, mode: str = "deep") -> Dict:
    """Analyze text using the /api/ai/analyze endpoint"""
    ...
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        ...
        payload = {
            "request_text": text,
            "mode": mode,
            "tier": "thinking"  # Use balanced tier
        }
...
async def test_document_format(file_path: str, test_case: TestCase, use_ai_analyze: bool = True):
    ...
    try:
        ...
        if use_ai_analyze:
            ...
            # Analyze using AI endpoint
            result = await analyze_text_with_ai(text_content)
            deliverables = result.get('suggestions', [])
            method_used = "ai_analyze"
        else:
            # Use file upload endpoint
            result = await upload_file_and_get_suggestions(file_path)
            deliverables = result.get('suggested', [])
            method_used = "suggest_by_file"
        ...
        # Check if GPT-5 was used (look for indicators in the response)
        is_gpt5 = deliverable_count >= MIN_DELIVERABLES_GPT5
        
        # Verify deliverable count
        if deliverable_count < MIN_DELIVERABLES_GPT5:
            test_case.warnings.append(f"Only {deliverable_count} deliverables returned (expected {MIN_DELIVERABLES_GPT5}+)")
            
        # Check for confidence and evidence in deliverables
        ...
        for d in deliverables:
            ...
            if 'confidence' in d or 'score' in d:
                has_confidence = True
            if 'evidence' in d or 'reasoning' in d or 'rationale' in d:
                has_evidence = True
            if 'department' in d:
                departments.add(d['department'])
            elif 'category' in d:
                departments.add(d['category'])
            elif 'Service' in d:  # For suggest_by_file response
                departments.add(d['Service'])
        ...
        # Determine pass/fail
        if deliverable_count >= MIN_DELIVERABLES_FALLBACK:
            if is_gpt5:
                test_case.complete("passed", test_result)
            else:
                test_case.warnings.append("Possibly using fallback method, not GPT-5")
...
async def test_error_handling(file_path: str, test_case: TestCase, should_fail: bool = True):
    ...
    try:
        if not os.path.exists(file_path):
            # If file doesn't exist and we expect failure, that's a pass
            if should_fail:
                test_case.complete("passed", {"error": "File not found as expected"})
            else:
                test_case.complete("failed", error="File not found")
            return
        ...
        if file_path.endswith('.txt'):
            ...
            if not text_content.strip():
                if should_fail:
                    test_case.complete("passed", {"error": "Empty file handled correctly"})
                else:
                    test_case.complete("failed", error="Empty file")
                return
            result = await analyze_text_with_ai(text_content)
        else:
            result = await upload_file_and_get_suggestions(file_path)
...
async def test_performance_comparison():
    ...
    try:
        ...
        with open("test_docs/luxury_fashion_rfp_small.txt", 'r', encoding='utf-8', errors='ignore') as f:
            text_content = f.read()
        
        result_small = await analyze_text_with_ai(text_content)
    ...
    try:
        ...
        with open("test_docs/luxury_fashion_rfp_large.txt", 'r', encoding='utf-8', errors='ignore') as f:
            text_content = f.read()
        
        result_large = await analyze_text_with_ai(text_content)
        end_mem = psutil.virtual_memory().percent
    ...
    if "small" in results and "large" in results:
        time_ratio = results["large"]["response_time"] / results["small"]["response_time"] if results["small"]["response_time"] > 0 else 0
...
async def run_all_tests():
    ...
    print("\n" + "="*60)
    print("📁 Testing Document Format Support (Text Analysis)")
    print("="*60)
    
    # Test different document formats using AI analyze endpoint (for text files)
    text_formats = [
        ("test_docs/luxury_fashion_rfp_small.txt", "TXT Format - Small (AI Analyze)"),
        ("test_docs/luxury_fashion_rfp_large.txt", "TXT Format - Large (AI Analyze)"),
    ]
    
    for file_path, description in text_formats:
        test_case = TestCase(f"AI Analyze - {description}", f"Test {description} document processing via text analysis")
        await test_document_format(file_path, test_case, use_ai_analyze=True)
    
    print("\n" + "="*60)
    print("📁 Testing Document Upload (File Suggestions)")
    print("="*60)
    
    # Test file upload endpoint for all formats
    upload_formats = [
        ("test_docs/luxury_fashion_rfp_small.txt", "TXT Upload - Small"),
        ("test_docs/luxury_fashion_rfp_small.docx", "DOCX Upload - Small"),
        ("test_docs/luxury_fashion_rfp_small.pdf", "PDF Upload - Small"),
        ("test_docs/luxury_fashion_rfp_large.txt", "TXT Upload - Large"),
        ("test_docs/luxury_fashion_rfp_large.docx", "DOCX Upload - Large"),
        ("test_docs/luxury_fashion_rfp_large.pdf", "PDF Upload - Large"),
    ]
    
    for file_path, description in upload_formats:
        test_case = TestCase(f"File Upload - {description}", f"Test {description} via file upload endpoint")
        await test_document_format(file_path, test_case, use_ai_analyze=False)
...
def generate_report(results: Dict):
    ...
    report_path = "test_report_gpt5_rfp_analysis_v2.md"
    
    with open(report_path, 'w') as f:
        f.write("# GPT-5 RFP Analysis - Test Report V2\n\n")
        ...
        if deliverable_tests:
            counts = [t['result']['deliverable_count'] for t in deliverable_tests if t['result'].get('deliverable_count', 0) > 0]
            if counts:
                ...
                f.write(f"- **Fallback Minimum:** {MIN_DELIVERABLES_FALLBACK}+\n\n")
...
async def main():
    ...
    try:
        ...
        with open("test_results_gpt5_rfp_analysis_v2.json", 'w') as f:
            json.dump(results, f, indent=2)
        
        print("\n✅ Testing completed successfully!")
        print(f"📊 Results saved to: test_results_gpt5_rfp_analysis_v2.json")
