...
class GovernanceFramework:
    ...
    def _generate_executive_briefings(self) -> List[TimelineTask]:
        ...
        # Find transitions between different departments
        # Group tasks by department and sort by start date to find transitions
        tasks_by_dept = {}
        for task in self.tasks:
            if task.department not in tasks_by_dept:
                tasks_by_dept[task.department] = []
            tasks_by_dept[task.department].append(task)
        
        # Sort tasks within each department by start date
        for dept in tasks_by_dept:
            tasks_by_dept[dept].sort(key=lambda t: datetime.fromisoformat(t.start_date))

        # Identify the earliest start date for each department that signifies a phase start
        phase_starters = {}
        for dept, dept_tasks in tasks_by_dept.items():
            if dept_tasks:
                phase_starters[dept] = datetime.fromisoformat(dept_tasks[0].start_date)

        # Sort phases by start date
        sorted_phases = sorted(phase_starters.items(), key=lambda item: item[1])
        ...
        for i in range(len(sorted_phases) - 1):
            ...
            milestone = TimelineTask(
                id=f"gov_exec_briefing_{from_dept}_{to_dept}",
    ...
    def _generate_quarterly_reviews(self) -> List[TimelineTask]:
        ...
        for q in range(quarters):  # All quarters
