  ...
  const ScenarioManager = {
    ...
    getCurrentScenario() {
      return {
        items: this.state.deliverables.map(d => ({
          ...
          hours: d.hours || 0,
          rate: d.rate || this.state.blendedRate || 210,
          blended_rate: d.rate || this.state.blendedRate || 210,
          effective_rate: d.rate || this.state.blendedRate || 210,
          price: d.price || 0,
          ...
          components: (d.components || []).map(c => ({
            name: c.title,
            hours: c.hours || 0,
            rate: c.rate || d.rate || this.state.blendedRate || 210,
    ...
    updateDeliverablesFromAPI(apiResponse) {
      ...
      scenario.items.forEach(item => {
        const deliverable = {
          ...
          rate: item.blended_rate || item.rate || item.effective_rate || 210,
          price: item.price || 0,
    ...
    buildScenarioPayload() {
      ...
      const payload = {
        ...
        blendedRate: this.state.blendedRate || window.getBlendedRateFromUI?.() || 210,
