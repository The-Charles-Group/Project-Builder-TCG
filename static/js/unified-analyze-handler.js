  ...
  function initUnifiedAnalyze() {
    ...
    newBtn.addEventListener('click', async function(e) {
      ...
      try {
        ...
        const hasFiles = window.FileStagingModule?.state?.stagedFiles?.length > 0;
        
        let rfpText = '';
        
        if (hasFiles) {
          console.log('[Unified Analyze] Extracting text from uploaded files...');
          ...
          if (extractRes.ok) {
            ...
            rfpText = data.text || '';
            console.log('[Unified Analyze] Extracted text:', rfpText.length, 'characters');
          } else {
            console.error('[Unified Analyze] Failed to extract text:', extractRes.status);
          }
        }
