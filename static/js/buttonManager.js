  ...
  const ButtonManager = {
    ...
    attachListeners() {
      ...
      for (const [selector, config] of Object.entries(this.buttons)) {
        ...
        if (element) {
          // Check if already wired to avoid duplicate listeners
          if (element.classList.contains('button-wired')) {
            console.debug(`[ButtonManager] Button already wired: ${selector}`);
            continue;
          }

          // Add listener with error boundary (use capture phase to ensure it fires first)
          ...
          element.classList.add('button-wired');
          element.dataset.buttonDescription = config.description;
          attached++;
        } else {
          missing++;
